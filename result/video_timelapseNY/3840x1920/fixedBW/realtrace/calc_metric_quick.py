...
method_arr = np.array([3,6,12])
#method_arr = np.array([20, 21])
mname_arr = np.array(['ROI', 'Ghent', 'Prob360']) 
        ...
        for bw in np.nditer(bw_arr):
            ...
            for trace in np.nditer(trace_arr):
                ...
                for method in np.nditer(method_arr):
                    ...
                    log =  "log_frame_TRACE_" + str(trace) + "_BW_" + str(bw)+ "_METHOD_" + str(method) + "_INTER_" + str(inter) + "_BUFF_" + str(buff) + "_EST_0.txt"
                    print log
                    f = open(log, "r")
                    # skip the first line
                    line = f.readline()
