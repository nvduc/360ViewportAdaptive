...
bw_arr = np.array([0])
method_arr = np.array([1, 20, 21, 23, 24])
#method_arr = np.array([20, 21])
mname_arr = np.array(['ROI', 'OPT-1', 'OPT-2', 'OPT-1-frame', 'OPT-2-frame']) 
...
for inter in np.nditer(inter_arr):
    for buff in np.nditer(buff_arr):
        for bw in np.nditer(bw_arr):
            log_metric = "metric_video_" + video + "_BW_" + dir_loc +  "_INTER_" + str(inter) + "_BUFF_" + str(buff) + "_S_" + str(START) + "_E_" + str(END) + ".txt"
            ...
            for trace in np.nditer(trace_arr):
                ...
                for method in np.nditer(method_arr):
                    ...
                    log =  "log_frame_TRACE_" + str(trace) + "_BW_" + str(bw)+ "_METHOD_" + str(method) + "_INTER_" + str(inter) + "_BUFF_" + str(buff) + "_EST_1.txt"
                    print log
                    f = open(log, "r")
                    # skip the first line
                    line = f.readline()
